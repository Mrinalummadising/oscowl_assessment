  ...
  const saveEdit = async () => {
    try {
      const response = await axios.put(
        `http://localhost:3001/api/todos/${editTaskId}`,
        {
          title: editTitle,
      ...
      setTodos(
        todos.map((todo) => (todo.id === editTaskId ? response.data : todo))
      );
      setEditTaskId(null);
      setEditTitle("");
      setEditDescription("");
  ...
  return (
    <>
      <nav className="navbar">
        ...
        <h2 className="navbar-title">Todo Application</h2>
        {token && (
          <button className="navbar-profile" onClick={handleProfile}>
            <CgProfile />
          </button>
        )}
        <button className="navbar-dashboard">Dashboard</button>
      </nav>

      <div className="todos-container">
        <h2 className="todos-title">Your Daily Tasks</h2>
